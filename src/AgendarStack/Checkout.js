...
const Checkout = () => {
  ...
  const getDaysInMonth = () => {
    ...
    return Array.from({ length: daysInMonth }, (_, i) => ({
      ...
      dayOfWeek: daysOfWeek[new Date(year, month - 1, i + 1).getDay()],
      isSelected: false,
    }));
  };
      ...
      <TouchableOpacity onPress={() => setSelectedDate(item.date)}>
        <View
          style={[
            ...
            { borderColor: selectedDate === item.date ? "#FF27A2" : "#7A7A7A" },
          ]}
        >
          <Text
            style={[
              styles.dateText,
              { color: selectedDate === item.date ? "#FF27A2" : "#7A7A7A" },
            ]}
          >
            {item.date}
          </Text>
          <Text
            style={[
              ...
              { marginTop: selectedDate === item.date ? 4 : 15 },
              { color: selectedDate === item.date ? "#FF27A2" : "#7A7A7A" },
            ]}
          >
            {item.dayOfWeek}
    ...
    <TouchableOpacity onPress={() => setSelectedTime(item.time)}>
      <View
        style={[
          ...
          { borderColor: selectedTime === item.time ? "#FF27A2" : "#7A7A7A" },
        ]}
      >
        <Text
          style={[
            styles.timeText,
            { color: selectedTime === item.time ? "#FF27A2" : "#7A7A7A" },
          ]}
        >
          {item.time}
        </Text>
        ...
        <ScrollView>
          {professionals.map((professional, index) => (
            <TouchableOpacity
              key={index}
              onPress={() =>
                navigation.navigate("DetalhesProfissional", { professional })
              }
            >
